...
export default function ProfilePage() {
    ...
    try {
      ...
      if (result.errorCode) {
        if (result.errorCode === 'permission') {
          ...
          showToast(result.errorMessage || "Fotoğraf seçilemedi", "error");
        }
        return;
      }
      ...
      if (result.assets && result.assets[0]) {
        ...
        // Convert to expo-image-picker compatible format
        const asset = {
        ...
        await uploadProfileImageMutation.mutateAsync(asset as any);
  ...
  const handleCompanyLogoUpload = async () => {
    try {
      ...
      if (result.didCancel) {
        logger.debug("🏢 Logo picker cancelled");
        return;
      }
      ...
      if (result.errorCode) {
        if (result.errorCode === 'permission') {
          ...
          showToast(result.errorMessage || "Logo seçilemedi", "error");
        }
        return;
      }
      ...
      if (result.assets && result.assets[0]) {
        ...
        await uploadCompanyLogoMutation.mutateAsync(asset as any);
